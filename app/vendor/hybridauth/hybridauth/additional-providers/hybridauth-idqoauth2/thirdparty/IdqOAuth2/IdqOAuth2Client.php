...
// A service client for the idQ OAuth 2 flow.
class IdqOAuth2Client
{
    public $api_base_url     = "";
    ...
    public function authenticate($code)
    {
        ...
        if (! $response || ! isset($response->access_token)) {
            throw new UnexpectedValueException("The Authorization Service returned the following error: " . $response->error);
        }
            ...
            if ($this->token_info_url && $this->refresh_token) {
                ...
                if ($tokeninfo && isset($tokeninfo->error)) {
                    ...
                    if (! isset($response->access_token) || ! $response->access_token) {
                        throw new UnexpectedValueException("The Authorization Service has returned an invalid response while requesting a new access token. Giving up!");
                    }
    ...
    */
    ...
    private function request($url, $params = false, $type = "GET")
    {
        ...
        if ($type == "POST") {
            ...
            // idQ OAuth 2 server requires POST submission of form data
    ...
    {
        Hybrid_Logger::debug("OAuth2Client::parseRequestResult(): ", $result);
