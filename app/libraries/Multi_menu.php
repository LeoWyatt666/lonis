...
class Multi_menu
{
    ...
    /**
     * Tag opener of the language
     * default is '<l>' tag
     *
     * @var string
     */
    private $lang_tag_open             = '<l>';
    
    /**
     * Closing tag of the language
     * default is '<l>' tag
     *
     * @var string
     */
    private $lang_tag_close            = '</l>';
     ...
     * Ex: http://localhost/./public/assets/images
     *
     * @var string
     */
    ...
    public function __construct($config = array())
    {
        ...
        $this->initialize($config['multi_menu']);
    }
    ...
    private function render_item($items, &$html = '')
    {
        ...
        foreach ($items as $item) {
            ...
            if (isset($item[$this->menu_label], $item[$this->menu_key])) {
                ...
                $label = $this->lang_tag_open . $label . $this->lang_tag_close;
                if ($icon) {
                    $icon = "<i class='{$icon}'></i>";
                ...
                $html .= $tag_open; //$this->set_active($tag_open, $slug);
    ...
    private function set_active($html, $slug)
    {
        ...
        if (($this->item_active != '' && $slug == $this->item_active && empty($segment)) || $slug == $segment) {
            ...
            return preg_replace('~<(?:!DOCTYPE|/?(?:html|body|\/div))[^>]*>\s*~i', '', $doc->saveHTML());
        }
