...
class Nested_set
{
    ...
    {
        $CI =& get_instance();
// to access CI resources, use $CI instead of $this
        $this->db =& $CI->db;
    }
    ...
    */
    ...
    public function getNodeWhere($whereArg = '"1"="1"')
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $resultNode = array_shift($result);
        // assumes CI standard $row[0] = first row
        }
    ...
    public function getNodeFromId($primarykey)
    {
        ...
        if (empty($this->primary_key_column_name)) {
            return false;
        }
    ...
    function buildCrumbs($crumbData)
    {
        ...
        foreach ($crumbData as $itemId) {
            ...
            $retVal .= '<li>' . anchor('shop/category/' . $itemId['id'], $itemId[$this->text_column_name], array(
    ...
    function buildMenu($parentId, $menuData, $depth = 0)
    {
        ...
        if (isset($menuData['parents'][$parentId])) {
            ...
            foreach ($menuData['parents'][$parentId] as $itemId) {
                $retVal .= '<li class="depth-' . $depth . '">' . anchor('shop/category/' . $menuData['items'][$itemId]['id'], $menuData['items'][$itemId][$this->text_column_name], array(
                        'class' => 'id-' . $itemId['id']
                    ));
